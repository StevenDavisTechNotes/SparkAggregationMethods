...
object method_rdd_reduce {
  def addRowToRows(lrows: mutable.ListBuffer[Record], rrow: Record, nInitialLRows: Int): mutable.ListBuffer[Record] = {
    var found = false
    for (lindex <- 0 until nInitialLRows) {
      if (!found) {
        ...
        if (Matching.IsMatch(
          ...
          lrows(lindex) = Matching.CombineRowList(Seq(lrow, rrow))
          val x = lrows(lindex)
          found = true
        }
      }
  ...
  def seqOpDisjoint(lrows: mutable.ListBuffer[Record], rrow: Record): mutable.ListBuffer[Record] = {
    lrows :+ rrow
  }
  def seqOpMixed(lrows: mutable.ListBuffer[Record], rrow: Record): mutable.ListBuffer[Record] = {
    addRowToRows(lrows, rrow, lrows.length)
  }
  def combOp(lrows: mutable.ListBuffer[Record], rrows: mutable.ListBuffer[Record]): mutable.ListBuffer[Record] = {
  ...
  def run(
    ...
    val seqOp = if (canAssumeNoDupesPerPartition) seqOpDisjoint _ else seqOpMixed _
