...
class Test_ProgressiveVariance:
    def test_default_option_with_unbiases_variance_and_along_no_axis(self):
        ...
        assert inc_mean_var.update(np.array([2, 8, 7, 4, 5])) == (5.2, 5.7)
        assert inc_mean_var.update(np.array([-3, 5, 2, 6])) == (4.0, 11.0)

    def test_biased_variance(self):
        inc_mean_var = ProgressiveVariance(ddof=0)
        assert inc_mean_var.update(np.array([2, 8, 7, 4, 5])) == (5.2, 4.5600000000000005)
        assert inc_mean_var.update(np.array([-3, 5, 2, 6])) == (4.0, 9.777777777777779)

    def test_along_axis_0(self):
        inc_mean_var = ProgressiveVariance(axis=0)
        result = cast(tuple[np.ndarray, np.ndarray],
                      inc_mean_var.update(np.array([
                          [2, 8, 7],
                          [-1, 10, 3],
                          [-7, 18, 5]
                      ])))
