...
from src.challenges.vanilla.vanilla_strategy_directory_dask import (
    VANILLA_STRATEGY_REGISTRY_DASK,
)
...
def parse_args() -> Arguments:
    ...
    strategy_names = sorted(x.strategy_name for x in VANILLA_STRATEGY_REGISTRY_DASK)
...
def do_test_runs(
    ...
    keyed_implementation_list = {
        x.strategy_name: x for x in VANILLA_STRATEGY_REGISTRY_DASK}
    keyed_data_sets = {x.data_description.size_code: x for x in prepare_data_sets(args)}
    with VanillaDaskRunResultFileWriter() as file:
    ...
    update_challenge_strategy_registration(
        ...
        registration=ChallengeResultLogFileRegistration(
            ...
            strategies=[
                ...
                for x in VANILLA_STRATEGY_REGISTRY_DASK
            ]
        ),
    )
