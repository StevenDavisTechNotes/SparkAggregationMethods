...
# prompt $g
# spark-submit --master local[*] --driver-memory 20g --deploy-mode client VanillaPerfTest.py
import gc
...
def vanilla_pandas_numpy(pyData):
    ...
    def inner_agg_method(dfPartition):
        ...
        return pd.DataFrame([[
            ...
            np.max(D),
            np.var(E),
            (np.inner(E,E) - np.sum(E)**2/E.count())/(E.count()-1),
            ]], columns=groupby_columns + agg_columns)
    #
    aggregates = df.groupby(df.grp, df.subgrp).apply(inner_agg_method)
    return None, aggregates
...
def vanilla_rdd_reduce(pyData):
    ...
    rddResult = sumCount.sortBy(lambda x: (x.grp, x.subgrp))
    return rddResult, None
...
def DoTesting():
    ...
    for cond_method in implementation_list:
        if cond_method.name not in ['vanilla_pandas']:
            continue
        for data in [pyData_3_3_10, pyData_3_3_100, pyData_3_3_1k, pyData_3_3_10k, pyData_3_3_100k]:
            cond_run_itinerary.extend((cond_method, data) for i in range(0, NumRunsPer))
    ...
    with open('Results/vanilla_runs.csv', 'a') as f:
        for index, (cond_method, data) in enumerate(cond_run_itinerary):
            ...
            f.write("%s,%s,%d,%f,%d\n"%(cond_method.name, cond_method.interface, result.dataSize, result.elapsedTime, result.recordCount))
            f.flush()
            df = None
            rdd = None
            gc.collect()
def DoAnalysis(python_implementation_list):
    ...
    with open('Results/vanilla_runs.csv', 'r') as f:
        for textline in f:
            ...
            if len(fields) < 5:
                fields.append('9')
            cond_method_name, cond_method_interface, result_dataSize, result_elapsedTime, result_recordCount = tuple(fields)
    ...
    with open('C:/Src/Blogging/BloggingSite/Content/Blog20180908ScalaAgg/Results/vanilla_runs_scala.csv', 'r') as f:
        for textline in f:
            if textline.startswith('#'):
                print("Excluding line: "+textline)
                continue
            ...
            if textline.find(',') < 0:
                ...
                continue
            fields = textline.rstrip().split(',')
            outcome,rawmethod,interface,expectedSize,returnedSize,elapsedTime = tuple(fields)
            if outcome != 'success':
                print("Excluding line: "+textline)
                continue
            if returnedSize != '9':
